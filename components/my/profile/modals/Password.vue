  ...
  <v-dialog :value="visible" transition="dialog-bottom-transition" persistent width="90vw">
    <v-card>
      ...
      <v-card-actions>
        ...
        <v-btn :disabled="!valid" :loading="loading" color="primary" @click="update">
          {{ $t('buttons.confirm') }}
        </v-btn>
      </v-card-actions>
    </v-card>
...
export default {
  ...
  computed: {
    ...mapState({
      loading: (state) => state.api.my.password_update.onFetch,
      error: (state) => state.api.my.password_update.error,
  ...
  methods: {
    ...mapActions({
      updatePassword: 'api/my/password_update/request',
      toast: 'toast/add',
